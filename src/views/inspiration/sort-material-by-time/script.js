...
function numberForm (idx, number) {
...
function convert2No (date) {
  var splitted = date.split(' ')
  var suffixIdx = date.toLowerCase().indexOf('ad')
  if (splitted.length === 3) return numberForm(suffixIdx, splitted[1])
  else return numberForm(suffixIdx, splitted[0])
}

mapData.bibleTimeline.forEach(time => {
  times.push(convert2No(time.date))
})
