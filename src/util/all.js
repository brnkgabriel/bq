...
var all = {
  ...
  utilities: {
    ...
    gotoPage: function (page, cPage, rPerPage, collection) {
      ...
      if (cPage < 1) { cPage = numPages } else if (cPage > numPages) { cPage = 1 }
    ...
    toggleModal: function () {
      if (this.modal.classList.contains('is-visible')) { this.loadedMaterial = null }
    ...
    search: function (toFind, collection, keys) {
      ...
      return collection.filter(item => {
        ...
        for (var j = 0; j < keys.length; j++) {
          var key = keys[j].split('-')[0]; var type = keys[j].split('-')[1]
          if (type === 'array') {
            if (idx(toFind, item[key].join(',')) !== -1) { condition = true }
          } else {
            if (idx(toFind, item[key]) !== -1) { condition = true }
          }
        }
        return condition
      })
  ...
  quiz: {
    processresponse: function (
      ...
      if (scoIdx === -1) { user.scores.push(state) } else { user.scores[scoIdx] = state }
    }
  },
  materials: {
